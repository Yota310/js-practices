import { openDatabase, runQuery, getQuery, closeDatabase } from './async_await_function.js';
...
async function main() {
    ...
    try {
        await runQuery(db, `CREATE TABLE books (
    ...
    let lastID;
    try {
        const result = await runQuery(db, `INSERT INTO books (title) VALUES (?)`, ['桃太郎'])
    ...
    if (lastID !== null) {
        try {
            const row =  await getQuery(db, `SELECT * FROM books WHERE id = ?`, [lastID]);
